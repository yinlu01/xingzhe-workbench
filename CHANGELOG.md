# 行者工作台更新日志

## [2026-09-04] feat(evolve): 进化存储层 + 双写通道（阶段 A/B）

**背景**：打卡数据只存在于浏览器 localStorage，自动化 Agent 读不到、也无法跨设备共享，导致「进化模块」（AI 每日改进建议）无数据可用。

- **新增 `evolution_store.py`**：SQLite 结构化分析库（Python 内置 `sqlite3`，零外部依赖）。
  - 事件溯源设计：`events` 事实表（只追加）+ `daily_metrics` 按天聚合视图，任何新指标都可重算。
  - CLI：`init` / `backfill`（幂等）/ `stats` / `query "<SQL>"`。
  - 已从 `review.json` 回填 2026-08-24 → 09-03 共 11 天历史打卡快照。
- **新增 `POST /api/event`**：前端双写通道。打卡时异步上报行为事件落库。
  - 硬约束：全程失败静默——服务端不可达、接口报错、存储层缺失都不影响任何打卡动作。
  - `evolution_store` 为可选依赖，导入失败时 server 照常提供服务。
- **前端接入 `trackEvent()`**（5 个入口）：今日阅读完成、阅读打卡、agent 学习打卡、运动打卡、复盘保存。
- **架构原则：共存不迁移**。localStorage 仍是主存储与唯一读取源，SQLite 纯为分析服务；不做 228 处调用点的全量重写。

**MVP 验证结论（用 11 天真实数据）**：

| 维度 | 完成率 |
|---|---|
| 写复盘 | 91% |
| 阅读打卡 / 运动 | 18% |
| agent 学习 | 9% |

→ 问题定性被数据推翻：不是「计划不好」，而是**高记录、低执行**。阅读最长停滞段 8/29–9/3 连续 5 天卡在 26%。进化模块的设计重心据此从「生成建议」调整为「把建议变成日程动作并追踪执行」。

**重要修正**：侧边栏「数据与手机版共享」此前并不成立——桌面 App（`127.0.0.1:8765`）与手机 PWA（`192.168.x.x:8765`）是不同 origin，localStorage 物理隔离。目前真正跨设备共享的只有走服务端的 `review.json`。双写通道为后续打通奠定基础。

**已知限制**：取消打卡暂不上报（只记录正向打卡）；待办完成未接入（`toggleTask` 内变量名与全局 `ts()` 冲突，留待后续）。

> 详细设计见 `设计文档-系统进化模块.md` v0.2。

## [2026-09-01] feat(skill): 每个技能配专属「五阶段训练页」（入门→练习→反馈→记录→精进）

- 背景：之前只有「表达训练」有专属工作台，其余技能点进去全是同一套「目标+3F+记录」通用模板，无技能专属逻辑。
- 新增五阶段训练框架，取代通用模板：**入门 → 练习 → 反馈 → 记录 → 精进**，每个已激活技能一套专属内容。
- 7 个技能专属内容（`SKILLS[].stages`）：
  - 表达训练：结论先行 / PREP·金字塔·SCQA / 对话式训练 / 优化点聚合
  - 力量训练：推拉腿三分化 / 渐进超负荷 / 记录重量×次数破 PR
  - 有氧训练：Zone2 心率区间 / 搏击操 / 配速心率趋势
  - 攀岩训练：V0-V1 起步 / 指力吊杠 / 难度递进 V3/V4
  - AI Agent 工程：四层架构 / 16 周冲刺 / 错题本 / 面试上岸
  - 深度阅读：费曼输出 / 四问回填 / 主题阅读
  - 人脉经营：价值交换 / 定期拜访 / 做连接者
- 交互整合：当前目标（练习板块）、3F 反馈本 + 福格断卡诊断（反馈板块）、练习记录（记录板块）、去练习按钮（精进板块后）全部保留，归位到对应阶段。
- 规划中的 5 个技能（结构化思维/精力管理/金融分析/写作/沟通协作）保持原样。

## [2026-09-01] fix(expr): 训练记录兼容手动打卡 + 全部记录可见

- 修复：「训练记录」视图之前只显示 `mode==='coach'` 的对话式训练，手动打卡记录被过滤掉，导致保存后"看不到任何记录"。
- 改为：训练记录 = **全部记录**（手动打卡 + 对话训练），按日期倒序展示最近 10 次。
- 记录卡适配：
  - 手动打卡 → 标注「打卡训练」、标签「表达内容」、不显示空的「问题」行。
  - 对话训练 → 标注「对话式训练」、标签「我的回答」、正常显示「问题」行。
- 技能档案页表达区同步兼容：全部记录均可出现在「最近 3 次训练」中。

## [2026-09-01] feat(expr): 表达训练升级为「对话式训练工作台」（语音 · 教练 · 自动沉淀）

### 核心设计：对话 = 训练场，工作台 = 记录本
- **训练在 WorkBuddy 对话里完成**：你说「开始表达训练」→ 果果出题（真实场景 / 当天所学）→ 你语音回答 30-60s → 获得诊断 / 优化版 / 优化要点 → 我写入工作台 SQLite（`wb_life_expr_ck`）。
- **工作台不增加语音输入**：WKWebView 做语音对话链路太重（改 Swift 壳 + 麦克风权限 + LLM 后端），而对话里发语音天然通。所以工作台只做展示与沉淀，训练在对话里。

### 表达面板三视图（打卡 / 对话训练 / 记录）
- **打卡训练**：原手动输入界面（PREP/金字塔/SCQA/自由框架），保留不变。
- **对话训练**：当天已有 `mode:'coach'` 记录时默认进入，展示五段式记录卡：
  - 问题 / 我的回答 / 果果反馈 / 优化版 / 优化要点（彩色标签区分）
- **训练记录**：全量历史 + 优化点聚合：
  - **优化点聚合**：按 `take` 字段频次统计 Top 5，一眼看出你最常被点到的表达问题。
  - **最近 10 次记录**：五段式卡片列表，日期倒序。

### 技能档案页 · 表达训练专属区
- 在通用档案页底部新增「🎙️ 对话式训练」section：
  - 高频优化要点聚合（Top 5）
  - 最近 3 次训练五段式卡
  - 「📜 查看全部训练记录」一键打开面板历史视图

### 数据与兼容
- `wb_life_expr_ck[date]` 增强字段：`mode`('free'/'coach')、`q`(问题)、`a`(我的回答)、`fb`(AI反馈)、`opt`(优化版)、`take`(优化要点)。
- 旧记录完全兼容（无 mode 视为自由打卡）。
- 手动打卡不再覆盖已有对话训练字段（`exprSave` 用 `Object.assign` 保留 coach 数据）。
- 连续天数/今日完成判定兼容 `a`（对话回答）和 `text`（手动打卡）双字段。

---

## [2026-08-31] feat(skill): 技能板块细化 —— 运动拆分为三类子技能 + 星球图标 + 字号放大

### 运动训练 → 拆分为三个子技能（P1 身体与精力）
- **💪 力量训练**：匹配 推日/拉日/腿日/核心/力量 类型打卡，历史基准 base=45 天 → L2 精通
- **🏃 有氧训练**：匹配 有氧/跑步/骑行/搏击操 类型打卡，历史基准 base=25 天 → L1 熟练
- **🧗 攀岩训练**：匹配 攀岩/吊杠 类型打卡，历史基准 base=12 天 → L1 熟练
- 运动打卡类型下拉新增「攀岩」选项。
- **机制**：新增 `exTpMatch()` 按打卡类型(tp)过滤，`skillTotalDays()` 叠加 `base`（历史训练基准，仅影响等级/累计天数，不影响连续天数），今日完成/连续天数按各自类型独立计算。
- 不再显示「新手」——力量/有氧/攀岩均按历史训练经验显示熟练/精通等级。

### 体验优化
- **左侧导航「技能」tab 图标**：星芒奖章 → 星球（行星 + 倾斜轨道环）。
- **技能板块整体字号 +1px**：列表页（分区标题/技能卡/进度条下方信息）与档案页（3F 反馈本/统计/练习记录/福格诊断）全部同步放大，图标块 32→34px。

---

## [2026-08-31] feat(skill): 新增「技能训练」板块（刻意练习 × 习惯养成）

### 新增：🎯 技能训练 tab（7 大板块之一）
- **四区分类**（按用户优先级）：
  - **P0 认知与表达**（最重要）：表达训练（已绑定）、结构化思维（规划中）
  - **P1 身体与精力**（其次）：运动训练（已绑定）、精力管理（规划中）
  - **P2 专业与创作**（深度）：AI Agent 工程（已绑定）、深度阅读（已绑定）、金融投资分析（规划中）、写作输出（规划中）
  - **P3 人际与领导**（广度）：沟通协作（规划中）、人脉经营（已绑定）
- **技能卡**：图标 + 名称 + 等级进度条（L0 新手→L1 熟练→L2 精通→L3 大师）+ 今日状态 + 连续天数/累计天数；规划中技能灰显。
- **今日统计**：右上角「今日 N/M」实时显示已绑定技能的完成进度。

### 技能档案页（点击已绑定技能进入）
- **等级体系**：按累计天数自动计算（0-6 天新手 / 7-29 天熟练 / 30-89 天精通 / 90+ 天大师），含升级路径提示。
- **当前目标**：可编辑输入，保存到 `wb_life_skill_meta`。
- **3F 反馈本**（刻意练习闭环）：
  - Focus：今天练什么？
  - Feedback：反馈来源？
  - Fix：下次改进？
  - 保存到 `wb_life_skill_log`，最近 7 条记录展示。
- **福格断卡诊断 B=MAP**：弹层三问（动机不足 / 能力不足 / 提示缺失）→ 自动生成针对性建议并记录。
- **去练习**：一键跳转现有模块（表达面板 / 运动页 / 读书页 / 人脉页）。

### 数据打通
- 表达训练 → 读 `wb_life_expr_ck`（打卡/字数/框架）
- 运动训练 → 读 `wb_life_exercises`
- AI Agent 工程 → 读 `wb_life_agent_ck`
- 深度阅读 → 读 `wb_life_plan_ck`
- 人脉经营 → 读 `wb_life_contacts`

### fix(home): 首页打卡圆圈完成态修复
- **Bug**：表达/运动/复盘三张首页卡片的圆圈完成态用了 `.done` 类，但 CSS 只定义了 `.rt-check.checked`，导致点击打卡后圆圈永远不会变绿。
- **修复**：三处 `check.classList.add('done')` → `add('checked')`，`remove('done')` → `remove('checked')`。

---

## [2026-08-31] feat(expr): 表达训练升级为「结构化表达工作台」（结论先行）

### 新增：表达框架选择（面板内置 4 种结构化模板）
- 基于主流表达方法论落地（搜索沉淀：麦肯锡 PREP、金字塔原理、SCQA 故事模型、电梯法则）：
  - **🧭 结论先行（PREP）**：结论 → 理由 → 例子 → 重申（默认，贴合"先说结论、观点明确"诉求）；
  - **🏔️ 金字塔**：结论置顶 → 2-3 并列分论点 → 论据 → 收束；
  - **💬 情境 SCQA**：情境 → 冲突 → 疑问 → 回答；
  - **✍️ 自由**：无模板，仍提示"第一句先给结论"。
- 选中框架后：文本框上方显示分步骨架引导（① ② ③ ④），placeholder 同步变为该框架的写作引导（结论/理由/例子/重申…）。
- 保存时记录所用框架 `wb_life_expr_ck[date].frame`；下次打开面板自动恢复上次框架 + 已写内容。

### 首页独立卡片（同日）
- 首页左侧新增「🎙️ 今日表达」rt-card（麦克风图标 #FF2D55），与读书/运动/复盘同级：
  - 未完成：`今日表达 · 30-60s 训练` / `30-60 秒讲清今天学到的一点` / `先输入→再输出→最后复盘`；
  - 已完成：`今日表达 · 连续 N 天` / `已完成 ✓ · 表达 N 字` / `点击回顾`；
  - 点击卡片/勾选直达表达面板；exprSave/取消打卡自动刷新。
- 新增 `exprStreakCount()`（表达连续打卡天数）。

### 其他（同日早前）
- 首页今日概览：表达从"21 点后提醒"改为全天常驻待办，与读书/学习/运动同级。

### 🗣️ 表达任务基础能力（同日早前）
### 新增：🗣️ 表达任务（每日例行，晚间 22:00-22:20）
- 每日计划晚间自动排入「表达：今日观点（30-60s）」任务，位于学习与复盘之间：先输入（读书/学习）→ 再输出（表达）→ 最后收尾（复盘）。
- 点击任务（勾选框或标题）打开「今日表达训练」面板：
  - 今日输入联动：自动带出今日读书章节/学习主题/今日复盘/昨日建议作为表达素材；
  - 30-60s 费曼输出引导（参考字数 30s≈80-120 字 / 60s≈160-240 字）+ 实时字数统计；
  - 保存后写入 `wb_life_expr_ck[date]={text,wc,ts}` 并标记任务完成，任务行显示「已表达 N 字 · 点击回顾」，点标题可随时回顾/再编辑。
- 打卡联动：`planSyncTodayFromCheckins` 双向同步、`ensurePlanForDate` 回填；取消打卡（再点一次勾选框）确认后清除当日记录。
- 不顺延（与复盘一致，仅 reading/study 顺延）。

### 联动与统计
- 系统提醒：21 点后未表达加入提醒文案；
- 月历新增表达圆点（#FF2D55）+ 图例「表达」；周聚焦条新增 🗣️ 图标；
- 手动添加任务类别新增「🗣️ 表达」；
- `selfcheck.py` 计划完整性检查升级为五类任务（读书/学习/运动/表达/复盘）。

### 技术
- `PLAN_SCHEMA 3→4`，已缓存「今天」计划自动失效重算，无需手动清数据。
- 数据键：`wb_life_expr_ck`（与 plan_ck/agent_ck 同级，复盘 snapshot 可读取）。

---

## [2026-08-29] fix+ui: 规划页日度视图（UTC 日期修复 + 布局稳定重构 + 比例缩小）

### 修复：UTC 日期错位（根因级）
- 全站 `toISOString().slice(0,10)` 是 UTC 时间，GMT+8 下本地零点 = UTC 前一天 16:00，导致：
  - 规划页日度「‹ ›」切日期错位（点后一天原地不动、点前一天跳两天）；
  - `ts()` 早上 8 点前取到「昨天」，影响打卡/计划生成/运动周统计/复盘热力图等 20 余处。
- 新增本地日期函数 `dstr()`，全量替换为本地日期口径。

### 修复：日度 hero 布局抖动
- 根因：读书页 `.plan-hero{display:flex}` 横向布局污染了规划页同名的 hero，头部/进度条/周条被并排放置。
- 加 `#planDayView .plan-hero{display:block}` 作用域隔离。
- 布局稳定四原则落地：固定宽度（日期标签/统计/按钮）、`visibility:hidden` 占位保留（今天按钮）、`tabular-nums` 等宽数字、周条文本固定（今天用实心圆标记，不再替换「日」为「今」）。
- 实测换 4 个日期坐标纹丝不动，周条 7 格精确等宽，读书页不受影响。

### 优化：日度页交互与比例
- 日期标签可点击弹出系统日期选择器；新增一周快捷跳转条（含完成度圆点）；键盘 ←/→ 翻日期。
- 「今天」按钮仅在非今日时显示（占位保留不位移）。
- 全区域按约 0.82 系数成比例缩小（hero 181px→135px、任务行/段间距/周条同步收紧），目标窗口一屏放下上午/下午/晚上三段，缩小后坐标稳定性复测通过。

---

## [2026-08-29] docs: 产品需求文档（PRD）v1.0

### 新增
- 新增 `产品需求文档.md`：产品定位、设计理念（进度驱动/单一数据源/本地优先等 6 条）、用户场景、六大模块功能需求（含 P0/P1/P2 优先级）、非功能需求、边界与路线图。
- 定位为产品演进主文档，结构按标准 PRD 组织，可直接抽取展示。

---

## [2026-08-28] feat(ops): 每日 7:00 轻量自测

### 新增
- 新增 `selfcheck.py`：每日 7:00 定时自测（已建 WorkBuddy 自动化任务），检查项：
  1. 读书游标 `wb_life_reading_cursor` 是否初始化；
  2. 今日计划是否生成且完整（读书/学习/运动/复盘四类任务）；
  3. 昨日复盘是否记录、AI 分析是否生成（读 `review.json`）。
- 只读 SQLite（`mode=ro`），不改数据，发现问题打印告警。

---

## [2026-08-27] fix(reading): 读书板块接入微信读书数据（在读书目 + 读书笔记）

### 修复
- 「在读书目」模块之前只读手动书目（`wb_life_books`，仅一条旧的"AI产品经理实战"），未反映真实在读。改为**优先显示微信读书 `inProgressBooks`**（含进度百分比 + 最近读日期），无微信读书数据时 fallback 手动书目。
- 「读书笔记」模块之前只读手动笔记（`wb_life_notes`，2 条旧笔记）。改为**优先显示微信读书 `recentNotes`**（划线/想法带标签 + 书名/章节/日期），无则 fallback 手动笔记。与首页「最近读书笔记」口径一致。

---

## [2026-08-27] fix(plan): 计划章节与游标一致性自愈

### 修复
- 新增 `ensurePlanForDate` 的**章节一致性自愈**：今天的读书/学习任务若与游标指向章节不一致（游标回退但计划仍是旧章节），自动失效重算，避免「游标和计划割裂」。
- 根因：之前 `ensurePlanForDate` 看到今天计划已是新 schema（v=3）就直接返回，游标变化后计划不跟着重算，导致首页今日概览等展示旧章节。

---

## [2026-08-27] fix(plan): 修复游标迁移误判 + 学习任务游标化

### 修复
- **游标迁移误判**：`migrateReadingCursor` 之前会把「今天的 done=True」也计入（多为微信读书自动打卡，不代表真正读完当前章节），导致游标跳到第 7 讲。修复：迁移时**跳过今天**，只统计历史真实完成的章节（→ 正确指向第 4 讲）。
- **学习任务游标化**：`ensurePlanForDate` 的 study 生成之前仍用旧顺延 `roll.study`（与 agent 游标矛盾），现改为直接用 agent 游标（`getTodayAgentInfo` 返回的当前学习日）。`planRolloverFrom` 已无调用（保留为死代码）。

---

## [2026-08-27] refactor(plan): 读书/学习计划改为「进度驱动」（整体平移）

### 背景
昨天的「顺延」只是单天顺延前一天，且读书章节仍由 `update_today.py` 按日历推进 `todaySchedule`——导致没读的章节被跳过、进度跳号（如今天跳到第 6 讲，实际应补第 4 讲）。用户要求的正确逻辑是：**未打卡就停留，后续整体往后平移**。

### 核心改动（进度游标）
- 读书、学习各维护一个「进度游标」（`reading_cursor` / `agent_cursor`）：**只有打卡完成才前进，未打卡停留** → 整体平移。
- 读书：新增 `buildReadingSequence()`（展平 67 周章节、跳过复习/休息）、`getTodayReadingChapter()`、`advance/rewindReadingCursor()`。`getTodayPlanInfo` / `ensurePlanForDate` / 读书页 hero / 首页 mini 卡**统一读游标章节**，废除 `todaySchedule` 割裂数据源。
- 学习（agent）：新增 `agent_cursor`（rest 日自动跳过）、`advance/rewindAgentCursor()`；`getTodayAgentInfo` 改为读游标（不再按日期匹配）。
- 打卡联动：规划页勾选 / 读书卡标记 / 微信读书自动打卡 / Agent 卡打卡，都会让游标前进；取消则回退。
- 迁移：首次运行自动初始化游标（读书游标从历史「最后完成章节」推断 → 第 4 讲；学习游标从打卡记录推断）。
- `PLAN_SCHEMA` 2→3，旧缓存自动失效重算。

---

## [2026-08-27] fix: 微信读书数据清零 + 读书章节错位

### Bug 1：微信读书数据清零
- **现象**：本月/本周/累计阅读时长、书架、笔记全部变为 0/空。
- **根因**：`weread_sync.py` 的 `call_api` 失败时返回空 dict，且无失败防御；8/27 08:44 定时同步时 API 瞬时返回空，脚本用空数据 `--save` 覆盖了 `weread_data.json`。
- **修复**：书架返回空（API 异常 / Key 失效 / 网络抖动）时 `sys.exit(1)` 中止，不再覆盖本地数据。数据已重新同步恢复（累计 137h58m / 本月 6h45m）。

### Bug 2：读书章节错位（今天显示第 5 讲，应为第 6 讲）
- **现象**：8/27（周四）读书显示「第 5 讲」（周三内容）。
- **根因**：`update_today.py` 由「行者创业系统每日读书提醒」在每天 21:00 才调用，导致当天白天 `todaySchedule` 还是昨天的，`ensurePlanForDate` 用昨天章节生成了今日计划。
- **修复**：新增「读书计划每日推进（凌晨）」自动化（每天 00:05 运行 `update_today.py`），确保当天一开始章节即正确；已手动推进 `todaySchedule` 到「第 6 讲 · 资本市场与直接融资」，App 自动刷新机制（`planRefreshToday`）已同步修正今日计划。

---

## [2026-08-26] feat(reading): 最近读书笔记接入微信读书 + AI 每日总结

### 问题
首页「最近读书笔记」模块只读 `wb_life_notes`（手动笔记，仅 2 条、停在 8/12），近期在微信读书的笔记/划线从未同步进来，导致模块「压根不更新」。

### 改动
- **数据层（`weread_sync.py`）**：新增 `fetch_recent_notes()`，拉取最近 8 本有笔记的书的划线（`/book/bookmarklist`）与想法（`/review/list/mine`）正文，过滤近 14 天，产出 `recentNotes` 字段（含 bookId/title/chapter/text/time/type）到 `weread_data.json`。
- **前端（`mac-dashboard.html`）**：首页「最近读书笔记」改为 `renderHomeNotes()`，优先展示微信读书近期笔记（划线/想法带标签），无数据时 fallback 手动笔记；新增「AI 读书笔记总结」区块（`homeNotesSummary`）。`autoLoadWeReadData` 与 `autoRefreshData` 同步后联动刷新首页。
- **AI 每日总结**：新增 `weread_summary.json`（结构 `{date, generatedAt, summary, highlights}`），前端轮询加载；新建 WorkBuddy 自动化「每日读书笔记总结」（每日 22:30），扫描近 3 天笔记生成总结写回。
- `weread_summary.json` 已加入 `.gitignore`（个人数据）。

---

## [2026-08-26] fix(reading): 读书页 hero 接入顺延逻辑（全站扫描收尾）

### 修复
- **读书页 hero（`renderReadingPlan`）此前仍直接读 reading_plan.json 的今日章节，未体现顺延**——这是全站最后一处残留的旧数据源展示。修复：hero 顺延优先，今日有 rolled 读书任务时显示「⏳ 顺延 · 阶段 · 第X周」+ 顺延书目/章节，无欠账时正常显示计划新章节。
- `planRefreshToday()` 增加 `renderReadingPlan()` 联动，使读书页 hero 在数据就绪重算后即时更新。

### 全站扫描结论（「今日读书/学习任务」展示入口统一收敛）
| 展示入口 | 状态 |
|---|---|
| 首页 · 今日概览 banner | 已接入每日计划 |
| 首页 · 读书卡 | 已接入 |
| 首页 · Agent 学习卡 | 已接入 |
| 首页 · mini 计划卡 | 已接入 |
| 任务页 · 读书卡 | 已接入 |
| 读书页 · hero | 本次接入 |
| 规划页 · 日/周月/待办 | 本就读每日计划 |

至此，所有「今日该做什么」的展示统一由 `getTodayDailyTasks()`（每日计划，含顺延）驱动，`getTodayPlanInfo()`/`getTodayAgentInfo()` 仅保留为数据源与 fallback。

---

## [2026-08-26] fix(plan): 休息日不再误生成学习任务 + 顺延机制梳理加固

### 修复
- **休息日边界**：agent 计划含 12 个周六 `rest` 日，此前 `ensurePlanForDate` 的 study 分支只判断 `ag.today` 存在、未排除 `type==='rest'`，导致休息日误生成「学习」任务。修复：`else if(ag&&ag.today&&ag.today.type!=='rest')`。顺延欠账仍会补（rest 日若有前一天未完成的学习任务仍顺延），但休息日不再凭空生成新学习任务。
- 读书位边界此前已正确（周日 `rest` 不生成读书、周六 `review` 生成复习），本次仅补齐 study 位，使两个位在「顺延优先 + 休息日不新增」上完全对称。

### 梳理结论（顺延机制完整性）
- 数据流闭环：`renderAll → renderPlan/首页 → ensurePlanForDate → planRolloverFrom(前一天) → 生成今日晚间`；`loadReadingPlan/loadAgentPlan → planRefreshToday` 兜底数据未就绪；`PLAN_SCHEMA` 版本号兜底代码升级；`planSyncTodayFromCheckins` 保证打卡与每日计划双向一致。
- 一致性入口：首页今日概览 / 读书卡 / Agent 卡 / mini 计划卡 / 规划页，均收敛到 `getTodayDailyTasks()` 单一数据源。

---

## [2026-08-26] fix(plan+home): 首页今日概览同步顺延 + 修复计划被永久缓存

### 新增（首页统一读取每日计划）
- 首页「今日概览」、读书卡、Agent 学习卡、读书计划 mini 卡统一改为**优先读取每日计划（含顺延任务）**，不再各自独立读 reading_plan.json / agent_study_plan.json 的「日期推进式」数据，顺延后首页与规划页完全一致。
- 顺延任务在首页以 `（⏳ 延期）` 标记呈现；mini 计划卡在读书顺延时显示「昨日未完成 · 顺延至今日，先补旧账」。
- 新增 `getTodayDailyTasks()`（首页统一入口）、`planDetailBrief()`（清洗任务 detail 前缀后缀）、`planSyncTodayFromCheckins()`（打卡与每日计划双向同步）。
- 打卡入口（`checkPlanToday` / `togglePlanCheckIn` / 微信读书自动打卡）在标记后触发 `renderHome()`，首页概览即时刷新。

### 修复（关键：计划被永久缓存导致更新不生效）
- **根因**：`ensurePlanForDate` 在初始化时于 reading_plan.json / agent_study_plan.json 异步加载完成前就生成了「今天」计划，并被 `generated:true` 永久缓存；之后代码升级（含顺延逻辑）也**不会触发重算**，所以首页始终显示旧章节、顺延从未真正计算。
- **方案 1（schema 版本）**：给计划数据加 `PLAN_SCHEMA` 版本号（当前=2），`ensurePlanForDate` 检测到 `v` 不符即视为旧缓存强制重算——**单纯重启 App 也能让代码更新生效**。
- **方案 2（数据就绪后刷新）**：reading_plan.json / agent_study_plan.json 加载成功后调用 `planRefreshToday()` 主动让今日计划失效并重算，确保顺延与章节基于最新数据。
- **方案 3（保留上午/下午）**：重算时仅重建晚间例行，保留用户已添加的上午/下午任务（修复上一版 `delete` 整条导致自定义任务丢失的回归）。

---

## [2026-08-26] feat(plan): 任务滚动顺延机制

### 新增
- **滚动顺延（Rollover）**：前一天晚间「读书/学习」任务若未完成，将自动顺延到今日晚间计划中，**不会再被跳过**。做完旧的才能推进新内容。
- 顺延任务标记：卡片显示 ⏳ 延期标签，detail 注明「未完成 · 自 X/X 顺延」，卡片带有浅橙底色提示。
- 新增辅助函数 `planPrevDate()`（跨月/跨年安全）和 `planRolloverFrom()`（提取欠账任务）。

### 修复
- `ensurePlanForDate` 从「每日重置」改为「顺延优先」：有欠账时不生成今日同类型新任务，仅补充顺延旧任务；无欠账时正常生成今日计划。

### 技术改动
- `ensurePlanForDate` 结构重构：先生成 `roll`（欠账），再按「读书位/学习位」判断顺延或今日新任务。
- `renderPlanSlot` 增加 `rolled` 标记渲染（`.plan-roll-tag` + `.plan-task.rolled`）。
- CSS 新增 `.plan-roll-tag`（橙色虚线芯片）和 `.plan-task.rolled`（浅橙渐变底色）。

---

## [2026-08-25] fix(plan): 修复添加任务数据丢失 + IME 回车误触

### 修复
- **Bug 1（数据丢失）**：`planEnsureDay` 内部重新读取 localStorage 导致返回新对象，`planConfirmAdd` 保存的却是旧对象，任务从未持久化。修复：给 `planEnsureDay` 增加可选 `daily` 参数，让写入方传入同一对象。`planApplySuggestion` 一并修复。
- **Bug 2（IME 回车误触）**：中文输入法选字时按回车被误判为「提交」。修复：输入框 `onkeydown` 增加 `isComposing || keyCode === 229` 守卫，仅真正编辑完才提交。

### 体验优化
- 添加成功后自动滚动到新任务并播放 `plan-flash` 高亮动画，解决「加完看不到」的体验问题。

---

## [2026-08-25] feat(plan): 美化规划页视觉

### 设计改动
- **Hero 渐变横幅**：橙→紫→蓝微渐变背景，大号完成百分比（如 20%）+ 完成计数 + 进度条缓动动画。
- **三段时段身份色**：上午（太阳图标·橙色）、下午（时钟图标·蓝色）、晚上（月亮图标·紫色），每段实时显示 `完成数/总数`。
- **任务卡片**：左侧 3px 分类色轨（读书蓝/学习紫/运动绿/复盘橙），hover 微抬升。
- **AI 建议卡片**：加装饰性光晕。
- **周月视图**：卡片标题加上身份色图标芯片，日历今日日期改为实心橙色圆点。

---

## [2026-08-25] refactor(plan): 合并规划与待办 Tab + 日历密度优化

### 优化
- **日历密度**：日历格从正方形改为紧凑行式（38px 高，日期左/圆点右），月历+双卡片可一屏内放下；去噪音（精简图例提示）。
- **Tab 合并**：侧栏去掉独立「待办」入口（badge 移至规划），规划页新增「待办」子 Tab（日度/周月/待办三视图），原核心目标+待办清单完整移入。

---

## [2026-08-24] feat(plan): 时间规划模块 V1

### 新增
- 规划页面：支持日度（上午/下午/晚上分段）、周月（合并视图）、待办三子视图。
- 自动生成晚间例行计划：读书（来自 reading_plan.json）、学习（来自 agent_study_plan.json）、运动、复盘、昨日 AI 建议。
- 支持任务添加/删除/完成/时间修改，打卡状态自动回填。
- 日历视图：周高亮、今日高亮、任务圆点标记。
